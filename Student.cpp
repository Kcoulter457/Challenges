...
yearofStudy_{ yearofStudy }, Person{ name }
{
	std::cout << "The second custom Student class constructor was called to create " << std::endl;
	numberofStudents_++;
...
std::ostream& operator<<(std::ostream& output, const Student& student) {
	output << " Registration: " << student.registrstionID_ << " Course: " << student.course_ << " Year of study: " << student.yearofStudy_ << std::endl << std::endl;
